...
The fact that we have reached an iteration where no privileges are present
  gives evidence that \Algorithm{IndSet} \Term{converges}.
...
Since the algorithm did what it said it was going to do,
  there is evidence that \Algorithm{IndSet} as \Term{closure}.

Obviously these properties would require \emph{formal} proof,
  but many examples of an algorithm succeeding would make it
  more \enquote*{worthwhile} to invest the time in its pursuit.
\todo[r/w]{Need a different word for worthwhile}
\begin{figure}[p]
  \centering
  \begin{tikzpicture}
